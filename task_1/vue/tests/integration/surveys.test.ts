...
describe("GET surveys/list", () => {
	...
	test("can filter by country code", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&country-code=US`)
		);
		expect(res.status).toBe(200);
		const json = await res.json();
		const returnedCountryCodes = Array.from(
			new Set(json.map((i: any) => i.countryCode))
		);
	...
	test("should return 400 when the country-code query parameter is not a 2-letter country code", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&country-code=USA`)
		);
		expect(res.status).toBe(400);
		const json = await res.json();
		expect(json.message).toBe(
			`"country-code" query parameter must be a 2-letter country code`
		);
	});
	...
	test("can filter by birth date range", async () => {
		const res = await fetch(
			new URL(
				`${baseApiUrl}/surveys/list?page=1&from-birth-date=2000-01-01&to-birth-date=2000-12-31`
			)
		);
		expect(res.status).toBe(200);
		const json = await res.json();
		...
		expect(
			birthDates.every(
				(i: any) =>
					new Date("2000-01-01") <= new Date(i) &&
					new Date(i) <= new Date("2000-12-31")
			)
		).toBe(true);
	});
	...
	test("can filter from birth date", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&from-birth-date=2000-01-01`)
		);
		expect(res.status).toBe(200);
		const json = await res.json();
		const birthDates = json.map((i: any) => i.birthDate);
		expect(
			birthDates.every((i: any) => new Date("2000-01-01") <= new Date(i))
		).toBe(true);
	});

	test("can filter to birth date", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&to-birth-date=2000-12-31`)
		);
		expect(res.status).toBe(200);
		const json = await res.json();
		const birthDates = json.map((i: any) => i.birthDate);
		expect(
			birthDates.every((i: any) => new Date(i) <= new Date("2000-12-31"))
		).toBe(true);
	});

	test("should return 400 when the from-birth-date query parameter is invalid", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&from-birth-date=invalid`)
		);
		expect(res.status).toBe(400);
		const json = await res.json();
		expect(json.message).toBe(
			`"from-birth-date" query parameter must be a date in the format "YYYY-MM-DD"`
		);
	});

	test("should return 400 when the to-birth-date query parameter is invalid", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&to-birth-date=invalid`)
		);
		expect(res.status).toBe(400);
		const json = await res.json();
		expect(json.message).toBe(
			`"to-birth-date" query parameter must be a date in the format "YYYY-MM-DD"`
		);
	});
	...
	test("should return 400 when the from-birth-date is greater than to-birth-date", async () => {
		const res = await fetch(
			new URL(
				`${baseApiUrl}/surveys/list?page=1&from-birth-date=2000-12-31&to-birth-date=2000-01-01`
			)
		);
		expect(res.status).toBe(400);
		const json = await res.json();
	...
	test("should return 400 when the name query parameter contains numbers", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&name=123`)
		);
		expect(res.status).toBe(400);
		const json = await res.json();
		expect(json.message).toBe(
			`"name" query parameter must not contain numbers`
		);
	});

	test("can filter by name", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&name=John`)
		);
		expect(res.status).toBe(200);
		const json = await res.json();
		expect(
			json.every(
				(i: any) => i.firstName.includes("John") || i.lastName.includes("John")
			)
		).toBe(true);
	});
	...
	test("can sort in ascending order", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&sort=asc`)
		);
		expect(res.status).toBe(200);
		const json = await res.json();
	...
	test("can sort in descending order", async () => {
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/list?page=1&sort=desc`)
		);
		expect(res.status).toBe(200);
		const json = await res.json();
...
describe("POST surveys/survey", () => {
	...
	test("should return 400 when the birth date is invalid", async () => {
		...
		const json = await res.json();
		expect(json.message).toBe(
			`"birthDate" must be a date in the format "YYYY-MM-DD"`
		);
	});
...
describe("DELETE surveys/survey/[id]", () => {
	...
	test("should return 204 when the survey is deleted", async (ctx) => {
		...
		const res = await fetch(
			new URL(`${baseApiUrl}/surveys/survey/${newCreatedSurveyId}`),
			{
				method: "DELETE",
			}
		);
