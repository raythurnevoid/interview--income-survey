...
const seed_data_content = fs.readFileSync(
	`${currentPath}/seed_data.csv`,
	"utf-8"
);
const seed_data = (
	...
	...r,
	birthDate: new Date(Number(`${r.birthDate}000`)).toISOString(),
...
await db.execute(
	sql`ALTER SEQUENCE ${sql.raw(getTableName(surveys))}_${sql.raw(surveys.id.name)}_seq RESTART WITH 1`
);
